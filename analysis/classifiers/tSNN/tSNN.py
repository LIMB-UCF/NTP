"""
Temporal spiking neural network training for full-length spike-train data.
"""
...
def gather_all_spike_files(spike_root: Path) -> List[Path]:
    """
    Recursively gather all spike .npz files from data/spike_trains/Texture*/.

    Returns a sorted list to ensure stable indexing for split indices.
    """
...
def pad_collate_fn(batch: List[Tuple[Optional[torch.Tensor], int]]) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
    """
    Collate that pads variable-length spike trains to max length in the batch.

    Input spikes: list of [C, T_i] -> output [B, C, T_max] float32
    """
    valid_spikes: List[torch.Tensor] = []
    labels: List[int] = []
    lengths: List[int] = []
    ...
    for spikes, y in batch:
        ...
        labels.append(y)
        lengths.append(int(spikes.shape[1]))

    if not valid_spikes:
        return None, None, None

    max_len = max(s.shape[1] for s in valid_spikes)
    padded: List[torch.Tensor] = []
    for s in valid_spikes:
        t = s.shape[1]
        if t < max_len:
            s = nn.functional.pad(s, (0, max_len - t))
        padded.append(s)

    x = torch.stack(padded, dim=0).float()
    y = torch.tensor(labels, dtype=torch.long)
    lengths_t = torch.tensor(lengths, dtype=torch.long)
    return x, y, lengths_t
...
def load_split(split_path: Path, all_files_sorted: List[Path], spike_root: Path) -> Tuple[List[Path], List[Path], List[Path]]:
    """
    ...
    Returns
    - (train_files, val_files, test_files) as lists of Path
    """
    ...
    raise ValueError(
        f"split.npz did not contain a recognized split format. "
        f"Expected indices (train_idx/val_idx/test_idx) or files (train_files/val_files/test_files)."
    )

class TemporalSNN(nn.Module):
    """Fully connected temporal SNN used for sequence classification."""

    def __init__(self, input_size: int, hidden1: int, hidden2: int, num_classes: int, beta: float):
        """Initialize the dataset."""
        super().__init__()
...
def batch_metrics(
    ...
    """Return class and texture-type accuracy counts for a batch."""
...
def run_one_epoch_train(
    ...
    """Train for one epoch and return loss, class accuracy, and type accuracy."""
    model.train()
    running_loss = 0.0
    running_weight = 0.0
    total = 0
    ...
    for bi, (x, y, lengths) in enumerate(pbar, start=1):
        if x is None:
            continue
...
def run_one_epoch_eval(
    ...
    """Evaluate for one epoch and return loss, class accuracy, and type accuracy."""
    model.eval()
    running_loss = 0.0
    running_weight = 0.0
    total = 0
...
def main() -> None:
    ...
    model = TemporalSNN(INPUT_SIZE, H1, H2, NUM_CLASSES, BETA).to(device)
