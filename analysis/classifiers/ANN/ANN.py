"""
Rate-coded artificial neural network baseline for full-length spike-train data.
"""
...
def raw_spikes_collate_fn(
    ...
    return spikes_list, torch.tensor(labels, dtype=torch.long), torch.tensor(lengths, dtype=torch.long)
...
def batch_to_window_tensor(
    spikes_list: List[torch.Tensor],
    window_size: int,
    stride: int,
    device: torch.device,
) -> Tuple[torch.Tensor, List[int]]:
    """Convert a batch of spike trains into concatenated rate windows."""
    windows: List[torch.Tensor] = []
    counts: List[int] = []

    for spikes_ct in spikes_list:
        w = spikes_to_rate_windows(spikes_ct, window_size=window_size, stride=stride)
...
class ANN(nn.Module):
    """Fully connected ANN used for rate-window classification."""
    ...
    def __init__(
        ...
        """Initialize a two-hidden-layer ReLU multilayer perceptron."""
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Return class logits for rate-coded input windows."""
...
def run_one_epoch_train(
    ...
    for _, (spikes_list, y, lengths) in enumerate(pbar, start=1):
        ...
        Xw, counts = batch_to_window_tensor(
            spikes_list,
...
def run_one_epoch_eval(
    ...
    for spikes_list, y, lengths in loader:
        if spikes_list is None:
            continue
...
def main() -> None:
    ...
    print("Model: two-hidden-layer ReLU ANN baseline")
    ...
    model = ANN(INPUT_SIZE, H1, H2, NUM_CLASSES).to(device)
