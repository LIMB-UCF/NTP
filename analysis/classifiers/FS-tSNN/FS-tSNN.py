"""
Temporal spiking neural network training for first-spike-truncated data.
"""
...
def pad_collate_fn(batch: List[Tuple[Optional[torch.Tensor], int]]) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
    """First-spike truncate and pad variable-length spike tensors."""
    ...
    for spikes, y in batch:
        ...
        fs = truncate_to_first_spike(spikes)
        if fs is None:
            continue

        valid_spikes.append(fs)
        labels.append(y)
        lengths.append(int(fs.shape[1]))
...
def run_one_epoch_train(
    ...
    for _bi, (x, y, lengths) in enumerate(pbar, start=1):
        if x is None:
            continue
